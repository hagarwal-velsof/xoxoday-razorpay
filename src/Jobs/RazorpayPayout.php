...
class RazorpayPayout implements ShouldQueue
{
    ...
    //construct takes the data passes to the job
    //$data has  $data = array("name" => "", "email" => "", "prefix" => "", "mobile" => "", "upi_id" => "", "reference_id" => "", 'amount' => "")
    ...
    public function createContact($name, $email, $mobile)
    {
        ...
        $data = array(
            ...
            'email' => $email,
            'contact' => $mobile,
            'type' => !empty(Config('xorazorpay.razorpay_contact_type')) ? Config('xorazorpay.razorpay_contact_type') : 'employee'
        );
